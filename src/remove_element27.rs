...
//The order of the elements may be changed. Then return the number of elements in nums which are not equal to
//val.
...
impl Solution {
    pub fn remove_element(nums: &mut [i32], val: i32) -> i32 {
        ...
        for i in 0..nums.len() {
            ...
            if nums[i] != val {
                //swap the element with the first element that is not equal to val
                nums[k] = nums[i];
                k += 1;
            }
        }
